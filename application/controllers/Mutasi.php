...
Class Mutasi extends CI_Controller{
	...
	public function ambilproses($idmaster){
		...
		if(empty($post['cc_tgl']) or empty($post['cc_jml'])){
			...
			if($post['cc_jml'] > $stokmaster){
				...
				$date = indo_date($post['cc_tgl']);
				$galonop = $this->mdmutasi->galon_op();	
				foreach ($galonop as $op) {
					$idop = $op['id'];
			
					if($post['cc_barang'] == $idop) {
						$nama_master = $this->mdmutasi->nama10($idop);
						$query = query("SELECT * FROM cc_terjual WHERE id_pelanggan = '$idmaster' AND id_master = '$idop' AND tgl = '$post[cc_tgl]'")->num_rows();
					}
				}

				if($query > 0){
					create_alert("Perhatian","Anda sudah melakukan pengambilan $nama_master pada $date , silahkan mengedit data pengambilan pada detail atau melakukan pengambilan di tanggal yang berbeda.","mutasi/ambil/$idmaster");
				}elseif($query == 0){	
	...
	public function hapus_detail($id_terjual,$idmaster)
	{
		$data = query("DELETE FROM cc_terjual WHERE id = '$id_terjual'");
		if($data > 0) {
			create_alert("Success","Data anda berhasil di hapus","mutasi/view/$idmaster");
		}
		else{
			create_alert("Gagal","Data anda gagal di hapus!","mutasi/view/$idmaster");
		}
	}

	public function simpan_edit($id_terjual)
	{
		$this->load->model('mdmutasi');
		$idmaster = $this->input->post('cc_pelanggan');
		$jml = $this->input->post('cc_jml');
		...
		$where = array('id' => $id_terjual);
		$update = $this->mdmutasi->update('cc_terjual',$data,$where);

		if($update > 0) {
			create_alert("Success","Data anda berhasil di update","mutasi/edit_detail/$id_terjual/$idmaster");
		}
		else{
			create_alert("Gagal","Data anda gagal di update!","mutasi/edit_detail/$id_terjual/$idmaster");
		}
	}

	public function edit_detail($id_terjual,$idmaster)
	{
		$curr = shortcode_login();
		$data['curr'] = $curr;

		$this->load->model("mdmutasi");
		$sql = $this->mdmutasi->master($id_terjual);
		foreach ($sql as $query) {
			$data['nama_master'] = $query->nama;
			$data['jml'] = $query->jml;
			$data['ket'] = $query->ket;
		}

		$data['harga'] = $this->mdmutasi->master1($id_terjual);
		$data['id_terjual'] = $id_terjual;
		$data['idmaster'] = $idmaster;
		$data['title'] = "Edit Barang : $data[nama_master]";
