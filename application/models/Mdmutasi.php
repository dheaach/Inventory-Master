...
Class Mdmutasi extends CI_Model{
	...
	{
		$query = $this->db->query("SELECT * FROM cc_pinjam_ambil WHERE id_pelanggan = '$idmaster' AND keterangan LIKE '%Pinjaman Awal%' OR keterangan LIKE '%Sisa Pinjaman%'");
		if($query->num_rows() == 1){
	...
	public function nama10($idop)
	{
		$sql = query("SELECT nama FROM cc_master WHERE id = '$idop'")->row_array();
		return $sql["nama"];
	}
	...
	public function master($id_terjual)
	{
	...
	public function master1($id_terjual)
	{
		$sql = query("SELECT harga FROM cc_terjual WHERE id = '$id_terjual'")->row_array();
		return $sql['harga'];
	}

	public function galon_op()
	{
		$sql = query("SELECT id FROM cc_master WHERE nama LIKE '%Galon%' AND id <> 1 AND harga > 0");
		return $sql->result_array();
	}
}