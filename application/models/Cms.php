...
Class Cms extends CI_Model{
	...
	public function get_page1($tb, $id){
		$sql = $this->db->query("SELECT * FROM $tb WHERE id_p = ".intval($id));
		if($sql->num_rows() > 0)
			return $sql->row_array();
		return false;
	}
	public function get_page2($tb, $id){
		$sql = $this->db->query("SELECT * FROM $tb WHERE id = ".intval($id));
		if($sql->num_rows() > 0)
			return $sql->row_array();
		return false;
	}
	public function get_page3($tb, $id){
		$sql = $this->db->query("SELECT * FROM $tb JOIN cc_master ON cc_master.id = cc_terima.id_master JOIN cms_admin ON cms_admin.id = cc_terima.id_admin WHERE id_master = ".intval($id));
		if($sql->num_rows() > 0)
			return $sql->row_array();
		return false;
	}
